...
class Puzzle:
    ...
    def solve(self):
        ...
        i_placement = 0
        while i_placement < 2 * N_ELEMENTS - 8:
    ...
    def calc_valids_for_first_placement(self, i_placement=None) -> bool:
        ...
        for i, combos in enumerate(self.combos_for_row):
            ...
            if min_n_valids is None or n_valids < min_n_valids:
                min_index, min_valids, min_n_valids, min_which = i, combos, n_valids, "row"

        # Cols:
        for i, combos in enumerate(self.combos_for_col):
            n_valids = len(combos)
            if min_n_valids is None or n_valids < min_n_valids:
                min_index, min_valids, min_n_valids, min_which = i, combos, n_valids, "col"

        self.valids_for_position[0] = min_valids
        self.placement_to_row_or_col[0] = min_index
        self.is_row_or_col[0] = min_which
    ...
    def calc_valids_for_nth_placement(self, i_placement) -> bool:
        ...
        if i_placement == 5:
            buena = (4, 6, 7, 3, 5, 1, 2)
            print("DEBUG290", buena in min_valids)
    ...
    def calc_valids_for_second_row(self) -> bool:
        i0 = self.placement_to_row_or_col[1]

        min_index, min_valids, min_n_valids = -1, [], None
        for i, combos in enumerate(self.combos_for_row):
            if i == self.placement_to_row_or_col[0]:  # avoid re-placing previous row
                continue
        ...
        self.placement_to_row_or_col[2] = min_index

        return True

    def calc_valids_for_second_col(self) -> bool:
        i0, i1 = self.placement_to_row_or_col[:3:2]  # 0, 2 (rows)

        min_index, min_valids, min_n_valids = -1, [], None
        for i, combos in enumerate(self.combos_for_col):
            if i == self.placement_to_row_or_col[1]:
                continue
        ...
        self.placement_to_row_or_col[3] = min_index

        return True

    def calc_valids_for_third_row(self) -> bool:
        i0, i1 = self.placement_to_row_or_col[1:4:2]  # 1, 3 (cols)

        min_index, min_valids, min_n_valids = -1, [], None
        for i, combos in enumerate(self.combos_for_row):
            if i in self.placement_to_row_or_col[:3:2]:  # 0 and 2  (rows)
                continue
        ...
        self.placement_to_row_or_col[4] = min_index

        return True

    def calc_valids_for_third_col(self) -> bool:
        i0, i1, i2 = self.placement_to_row_or_col[:5:2]  # indices for rows 0, 2, 4

        min_index, min_valids, min_n_valids = -1, [], None
        for i, combos in enumerate(self.combos_for_col):
            if i in self.placement_to_row_or_col[1:4:2]:  # indices for cols 1 and 3
                continue
        ...
        self.placement_to_row_or_col[5] = min_index

        return True

    def calc_valids_for_fourth_row(self) -> bool:
        i0, i1, i2 = self.placement_to_row_or_col[1:6:2]  # 1, 3, 5 (cols)

        min_index, min_valids, min_n_valids = -1, [], None
        for i, combos in enumerate(self.combos_for_row):
            if i in self.placement_to_row_or_col[:5:2]:  # 0, 2, 4 (rows)
                continue
        ...
        self.placement_to_row_or_col[6] = min_index

        return True

    def calc_valids_for_fourth_col(self) -> bool:
        i0, i1, i2, i3 = self.placement_to_row_or_col[:7:2]  # 0, 2, 4, 6 (rows)

        min_index, min_valids, min_n_valids = -1, [], None
        for i, combos in enumerate(self.combos_for_col):
            if i in self.placement_to_row_or_col[1:6:2]:  # 1, 3, 5 (cols)
                continue
        ...
        self.placement_to_row_or_col[7] = min_index

        return True

    def calc_valids_for_fifth_row(self) -> bool:
        i0, i1, i2, i3 = self.placement_to_row_or_col[1:8:2]  # 1, 3, 5, 7 (cols)

        min_index, min_valids, min_n_valids = -1, [], None
        for i, combos in enumerate(self.combos_for_row):
            if i in self.placement_to_row_or_col[:7:2]:  # 0, 2, 4, 6 (rows)
                continue
        ...
        self.placement_to_row_or_col[8] = min_index

        return True

    def calc_valids_for_fifth_col(self) -> bool:
        i0, i1, i2, i3, i4 = self.placement_to_row_or_col[0:9:2]  # 0, 2, 4, 8, 10 (rows)

        min_index, min_valids, min_n_valids = -1, [], None
        for i, combos in enumerate(self.combos_for_col):
            if i in self.placement_to_row_or_col[1:8:2]:  # 1, 3, 5, 5 (cols)
                continue
        ...
        self.placement_to_row_or_col[9] = min_index

        return True

    def calc_valids_for_sixth_row(self) -> bool:
        i0, i1, i2, i3, i4 = self.placement_to_row_or_col[1:10:2]  # 1, 3, 5, 7, 9 (cols)

        min_index, min_valids, min_n_valids = -1, [], None
        for i, combos in enumerate(self.combos_for_row):
            if i in self.placement_to_row_or_col[:9:2]:  # 0, 2, 4, 6, 8 (rows)
                continue
        ...
        self.placement_to_row_or_col[10] = min_index

        return True

    def calc_valids_for_sixth_col(self) -> bool:
        i0, i1, i2, i3, i4, i5 = self.placement_to_row_or_col[:11:2]  # 0, 2, 4, 6, 8, 10 (rows)

        min_index, min_valids, min_n_valids = -1, [], None
        for i, combos in enumerate(self.combos_for_col):
            if i in self.placement_to_row_or_col[1:10:2]:  # 1, 3, 5, 7, 9 (cols)
                continue
        ...
        self.placement_to_row_or_col[11] = min_index

        return True

    def calc_valids_for_seventh_row(self) -> bool:
        i0, i1, i2, i3, i4, i5 = self.placement_to_row_or_col[1:12:2]  # 1, 3, 5, 7, 9, 11 (cols)

        min_index, min_valids, min_n_valids = -1, [], None
        for i, combos in enumerate(self.combos_for_row):
            if i in self.placement_to_row_or_col[:11:2]:  # 0, 2, 4, 6, 8, 10 (rows)
                continue
        ...
        self.placement_to_row_or_col[12] = min_index

        return True

    def check_seventh_col(self) -> bool:
        already_placed_cols = self.placement_to_row_or_col[1:12:2]
        missing_col = 21 - sum(already_placed_cols)  # 15 = 0 + 1 + 2 + 3 + 4 + 5 + 6

        combo_must_be = [None for _ in range(N_ELEMENTS)]
        for i, ix in enumerate(self.placement_to_row_or_col[:13:2]):  # 0, 2, 4, 6, 8, 10, 12 (all rows)
